        ...
        <main class="main-container">
            ...
            <template v-if="payload.role == 'ADMIN' || payload.role == 'CHEF'">
                ...
                <div class="mb-6">
                    ...
                    <div class="mb-6">
                        <label class="block mb-2 text-sm font-medium text-gray-900 text-gray-300">Category name</label>
                        <input v-model="categoryName" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Category name" required="" />
                    </div>
                    <button type="submit" v-on:click="newCategory" class="text-white transition-all bg-blue-400 hover:bg-blue-500 focus:outline-none font-medium rounded-lg text-sm w-full sm:w-auto px-8 py-2.5 text-center">Submit</button>
                </div>
            </template>
            <div class="flex items-center mb-4 text-green-700 font-bold text-lg uppercase">
                <ThemifyIcon icon="menu" />
