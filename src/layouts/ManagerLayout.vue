<template>
    ...
    <div v-show="counter.Show_Table_Hoa_don == false" class="flex flex-col justify-center items-center fixed inset-0 z-50 h-screen w -screen bg-slate-800 gap-5">
        ...
        <div class="flex gap-2">
            <router-link to="/" class="font-semibold px-2 text-gray-200 py-1 rounded">Đăng nhập bàn</router-link>
            <router-link to="/manager" class="text-gray-200 border-b-[2px] border-lime-500 font-semibold px-2 py-1 rounded">Đăng nhập NVQL</router-link>
            <router-link to="/chef" class="text-gray-200 font-semibold px-2 py-1 rounded">Đăng nhập NVB</router-link>
            <!-- <h1>{{ counter.Path_Route.path }}</h1> -->
        </div>
